...
//	$queue_id = 1;
	...
	if($cmd == "getanswers" and $queue_id >=0)
	{
		...
		$query = "select Answers.id as answers_id, Answers.user_id, Answers.content, Answers.score, Answers.created_at, Answers.updated_at from Answers where question_id = ". $queue_id;
		$res = $db->query($query); 
		
		$answersResult = array();
		//| answers_id | user_id | content| score | created_at| updated_at|
		while ($r = mysqli_fetch_row($res)) { // important command
			array_push($answersResult,array("answers_id"=>$r[0],"user_id"=>$r[1],"content"=>$r[2],"score"=>$r[3],"created_at"=>$r[4],"updated_at"=>$r[5]));
		}
